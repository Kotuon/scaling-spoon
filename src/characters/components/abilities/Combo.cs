...
public partial class Combo : Ability
{
    ...
    protected Area2D damageArea
    {
        private set => _damageArea = value;
        get
        {
            if (_damageArea == null)
            {
    ...
    public override void Update(double delta)
    {
        ...
        if (animHandler.GetCurrentAnimation().Find(abilityName) == -1)
        {
            if (triggerNext)
            {
                var inputDirection = parent
                    .GetComponent<Controller>()
                    .moveInput;
                UpdateHitbox(inputDirection);
                animHandler.PlayAnimation(
                    abilityName + "_" + currAttack.ToString(),
                    inputDirection
                );
    ...
    {
        if (body is IDamageable && !bodiesInDamageArea.ContainsKey(body.Name))
        {
